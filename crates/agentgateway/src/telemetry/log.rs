...
#[cfg(feature = "adobe")]
use crate::telemetry::metrics::{MCPCallAdobe, MCPUpstreamError};
...
impl RequestLog {
	...
	/// Score health for an intermediate retry attempt and consume the request handle.
	/// Must be called after each non-final attempt so the provider can be evicted before
	/// the next `select_provider()` call picks the fallback group.
	pub fn finalize_for_retry(
		&mut self,
		...
		let default_unhealthy = status.is_none_or(|s| s.is_server_error());
		let unhealthy = if let Some(policy) = &self.health_policy {
			if let Some(expr) = &policy.unhealthy_expression {
				cel_exec.executor.eval_bool(expr.as_ref())
			} else {
				default_unhealthy
			}
		...
		let (health, eviction_duration, restore_health) = if let Some(policy) = &self.health_policy {
			let fallback_duration = retry_after.or(self.retry_backoff);
			policy.eviction_decision(current_health, consecutive_failures, times_ejected, unhealthy, fallback_duration)
		} else {
			(!unhealthy, None, None)
		};
...
impl Drop for DropOnLog {
	fn drop(&mut self) {
		...
		let llm_response = log
			.llm_response
			.take()
			.map(|llm| crate::cel::LLMContext::from_llm_info(llm, None));
		...
		#[cfg(feature = "adobe")]
		// Adobe-only: warn if upstream_error was stashed but method_name was never set —
		// the counter would be silently dropped by the method_name guard below.
		if let Some(mcp) = &mcp
			&& mcp.upstream_error.is_some() && mcp.method_name.is_none()
		{
			tracing::debug!(
				upstream_error = ?mcp.upstream_error,
				"mcp_upstream_errors_total not emitted: method_name absent"
			);
		}
			...
			&& mcp.method_name.is_some()
		{
			...
			#[cfg(feature = "adobe")]
			{
				let adobe_call = MCPCallAdobe {
					server: mcp.backend_name.as_deref().map(RichStrng::from).into(),
					target: mcp.target_name().map(RichStrng::from).into(),
					method: mcp.method_name.as_ref().map(RichStrng::from).into(),
					resource_type: mcp.resource_type().into(),
					resource: mcp.resource_name().map(RichStrng::from).into(),
					route: route_identifier.clone(),
				...
				// Adobe-only: count genuine upstream/transport failures, classified
				// and stashed by single-target dispatch (handler.rs). Emits only when
				// a transport error_type was recorded — success, RBAC denial,
				// client/protocol errors, and fan-out all leave this `None`.
				if let Some(t) = mcp.upstream_error.as_deref() {
					log
						.metrics
						.mcp_upstream_errors
						.get_or_create(&MCPUpstreamError {
							...
							target: mcp.target_name().map(RichStrng::from).into(),
							method: mcp.method_name.as_ref().map(RichStrng::from).into(),
							error_type: Some(RichStrng::from(t)).into(),
							route: route_identifier.clone(),
						})
						.inc();
				}
...
impl PolicyGrpcLogExporter {
	fn new(
		...
		let channel = GrpcReferenceChannel {
			...
			client: crate::proxy::httpproxy::PolicyClient { inputs, outbound: None },
		};
		let tonic_client =
